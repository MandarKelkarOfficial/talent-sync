...
//             <span className="rounded-xl bg-gray-200 p-2">Latest Performance by Topic:</span>
//           </h4>
//           <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
//             {latest.scoresByTopic.map(topic => {
//               const prevTopic = previous?.scoresByTopic?.find(p => p.topic === topic.topic);
...
//                   {
//                     topic.topic !== "Personality" && (
//                       <div className="bg-gray-50 p-3 rounded-lg shadow-sm transition-all duration-200 hover:shadow-md">
//                         <div className="font-medium text-gray-700 truncate">{topic.topic}</div>
//                         <div className="flex items-center justify-between mt-1">
//                           <span className="font-bold">{currentPerc}%</span>
//                           {prevPerc !== undefined && (
//                             <span
//                               className={`${currentPerc > prevPerc ? "text-green-600" : currentPerc < prevPerc ? "text-red-600" : "text-gray-600"} text-xs`}
//                             >
//                               {currentPerc > prevPerc ? "+" : ""}
//                               {currentPerc - prevPerc}%
//                             </span>
...
//       <button
//         onClick={onStart}
//         className=" w-full px-8 py-4 bg-gradient-to-r from-indigo-200 via-indigo-300 to-purple-200 text-purple-900 font-medium rounded-xl shadow-md transition-colors duration-300 ease-in-out transform-gpu hover:-translate-y-1 hover:shadow-lg hover:from-purple-600 hover:to-purple-700 hover:text-white focus:outline-none focus:ring-4 focus:ring-purple-200 active:scale-95"
//       >
//         {previousResults && previousResults.length > 0 ? 'Start New Test' : 'Start Test'}
//       </button>
//     </div>
...
//   const handleNext = () => {
//     markVisited(currentQ);
//     if (currentQ < test.questions.length - 1) {
//       setCurrentQ((q) => q + 1);
//       markVisited(currentQ + 1);
//     }
//   };
...
//       {/* MAIN QUESTION AREA */}
//       <div className="bg-white p-8 rounded-xl shadow-lg max-w-2xl w-full transition-all duration-300">
...
//       <div className="w-52 bg-white p-4 rounded-xl shadow-lg h-fit sticky top-6">
...
//             let colorClass = "bg-gray-300 text-gray-800"; // default = not attempted
//             if (isAnswered) colorClass = "bg-green-500 text-white"; // answered = green
//             else if (isVisited && !isAnswered) colorClass = "bg-red-500 text-white"; // visited but not answered = red
...
//                   markVisited(index);
//                 }}
//                 className={`w-10 h-10 rounded-lg flex items-center justify-center text-sm font-semibold ${colorClass} hover:opacity-90 transition`}
...
//           <div className="flex items-center gap-2">
//             <span className="w-3 h-3 inline-block bg-green-500 rounded-md"></span>
//             <span>Answered</span>
//           </div>
//           <div className="flex items-center gap-2 mt-1">
//             <span className="w-3 h-3 inline-block bg-red-500 rounded-md"></span>
//             <span>Visited — not answered</span>
//           </div>
//           <div className="flex items-center gap-2 mt-1">
//             <span className="w-3 h-3 inline-block bg-gray-300 rounded-md"></span>
...
// // --- Minimal in-file placeholder for ProgressDashboard ---
// // Remove this if you use your external ProgressDashboardApptitude component
// const ProgressDashboard = () => (
//   <div className="bg-white p-4 rounded-xl shadow-sm">
//     <div className="flex items-center justify-between">
//       <div>
//         <p className="text-xs text-gray-500">Progress</p>
//         <p className="font-medium text-gray-800">Streak: 3 tests</p>
//       </div>
//       <div className="text-right">
//         <p className="text-xs text-gray-500">Avg Score</p>
//         <p className="font-bold text-indigo-600">72%</p>
//       </div>
//     </div>
//   </div>
// );
...
// // --- Previous Results Panel (top of start screen) ---
// const PreviousResultsPanel = ({ results, onViewDetails }) => {
//   if (!results || results.length === 0) {
//     return (
//       <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-6 mb-8 shadow-md">
...
//       <div className="mt-6 pt-4 border-t border-gray-100">
//         <p className="text-sm text-gray-600 ">
//           <span className='rounded-xl bg-gray-200 p-2'> Last test taken: {new Date(results[0].completedAt).toLocaleDateString('en-US', {
//             year: 'numeric', month: 'long', day: 'numeric'
//           })}</span>
//         </p>
//       </div>
...
//         <div className="space-y-4">
//           {results.map((result, index) => (
//             <div key={result._id || index} className="border border-gray-100 rounded-xl p-5 shadow-sm hover:shadow-md transition-all duration-300">
//               <div className="flex justify-between items-start">
//                 <div className="flex-1">
//                   <div className="flex items-center gap-4 mb-2">
//                     <span className="text-2xl font-bold text-indigo-600">{result.overallScore}%</span>
...
// // --- QuestionScreen (UPDATED: 70% question / 30% right-side status panel) ---
// const QuestionScreen = ({ test, onSubmit, startTime }) => {
//   const [currentQ, setCurrentQ] = useState(0);
//   const [answers, setAnswers] = useState([]);
...
//   // mark visited for current question
//   useEffect(() => {
...
//   const notAttemptedCount = answers.filter(a => a === null).length;
//   const seenNotSolved = visited.filter((v, i) => v && answers[i] === null).length;

//   return (
//     <div className="flex flex-col md:flex-row gap-6 w-full">

//       {/* LEFT SIDE - QUESTIONS 70% */}
//       <div className="md:w-[70%] bg-white p-8 rounded-xl shadow-lg">
//         <div className="mb-4 flex justify-between items-center">
//           <div>
//             <span className="text-sm font-semibold text-indigo-600">
//               {question.topic}
...
//               <label
//                 key={index}
//                 className={`flex items-center p-4 rounded-lg cursor-pointer transition-all duration-200 ${answers[currentQ]?.answer === option ? 'bg-indigo-50 shadow-md border border-indigo-100' : 'bg-gray-50 hover:bg-gray-100 shadow-sm'}`}
//               >
//                 <input
//                   type="radio"
//                   name={`question_${currentQ}`}
//                   value={option}
//                   checked={answers[currentQ]?.answer === option}
//                   onChange={() => handleAnswer(option)}
//                   className="h-4 w-4 text-indigo-600 border-gray-300 focus:ring-indigo-500"
//                 />
//                 <span className="ml-3 text-gray-700">{option}</span>
...
//       {/* RIGHT SIDE STATUS PANEL 30% */}
//       <div className="md:w-[30%] bg-white p-6 rounded-xl shadow-lg h-fit sticky top-4">
//         <h3 className="text-lg font-bold text-gray-800 mb-4">Question Status</h3>

//         {/* GRID OF QUESTION NUMBERS (5 per row) */}
//         <div className="grid grid-cols-5 gap-3 mb-6">
//           {test.questions.map((_, idx) => {
//             const answered = answers[idx] !== null;
//             const visitedButNotAnswered = visited[idx] && !answered;
...
//                 className={`p-2 rounded-lg font-bold text-center ${bgColor} hover:opacity-90`}
//                 title={`Q${idx + 1} - ${answered ? 'Answered' : visitedButNotAnswered ? 'Seen (Not Solved)' : 'Not Attempted'}`}
//               >
//                 {idx + 1}
//               </button>
//             );
...
//             Attempted: <span className="font-bold">{attemptedCount}</span>
//           </p>
//           <p className="font-medium text-red-700">
//             Not Attempted: <span className="font-bold">{notAttemptedCount}</span>
//           </p>
//           <p className="font-medium text-yellow-700">
//             Seen but Not Solved: <span className="font-bold">{seenNotSolved}</span>
//           </p>
//         </div>
//       </div>
//     </div>
...
// // --- Improvement Highlights used in ReportScreen ---
// const ImprovementHighlights = ({ current, previous }) => {
//   if (!previous) return null;
...
// // --- Report Screen (after finishing test) ---
// const ReportScreen = ({ result, previousResults }) => {
//   const previousResult = previousResults?.[0];
...
//                   <span className="text-sm text-gray-500">({currentPerc}%)</span>
//                   {prevPerc && (
//                     <span className={`text-xs px-2 py-1 rounded-full ${currentPerc > prevPerc ? 'bg-green-100 text-green-700' : currentPerc < prevPerc ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-700'}`}>
//                       {currentPerc > prevPerc ? '+' : ''}{currentPerc - prevPerc}%
//                     </span>
//                   )}
//                 </div>
...
// export default function AptitudeTest() {
//   const [testState, setTestState] = useState('idle'); // 'idle', 'generating', 'in-progress', 'submitting', 'completed'
//   const [testData, setTestData] = useState(null);
//   const [result, setResult] = useState(null);
//   const [error, setError] = useState('');
//   const [previousResults, setPreviousResults] = useState([]);
...
// /* -------------------------
//    QuestionScreen with Proctoring
//    ------------------------- */
...
//   const MAX_WARNINGS = 3; // terminate after this many warnings
//   const DETECTION_INTERVAL_MS = 1500; // run detection every 1.5s
...
//   // Mark visited when question changes
//   useEffect(() => {
//     setVisited(prev => {
//       const next = [...prev];
//       next[currentQ] = true;
...
//   // Load face-api models from /server/models
//   useEffect(() => {
//     let mounted = true;
//     const load = async () => {
//       try {
//         // loads tiny face detector model from server
//         await faceapi.nets.tinyFaceDetector.loadFromUri('/server/models');
//         // Optionally load landmarks if you want (not required for presence detection)
//         await faceapi.nets.faceLandmark68TinyNet.loadFromUri('/server/models').catch(() => {});
//         console.log('face-api models loaded from /server/models');
//       } catch (err) {
...
//         // give time for video + models to be ready
...
//             }
//           } catch (e) {
//             // models might not be ready, ignore gracefully
//           }
...
//   // Disable copy/paste/context/select & some devtools keys (best-effort)
//   useEffect(() => {
...
//   // Terminate on too many warnings
//   useEffect(() => {
...
//       setTimeout(() => {
//         alert('Test terminated due to suspicious activity (multiple warnings).');
...
//   const handleNext = () => {
//     if (currentQ < test.questions.length - 1) setCurrentQ(c => c + 1);
//   };
//   const handlePrevious = () => {
...
//         <p className="text-lg text-gray-800 mb-6 select-none" style={{ userSelect: 'none' }}>{question.questionText}</p>
...
//                   className="h-4 w-4 text-indigo-600 border-gray-300 focus:ring-indigo-500"
//                 />
//                 <span className="ml-3 text-gray-700 select-none" style={{ userSelect: 'none' }}>{option}</span>
//               </label>
//             ))}
//           </div>
//         ) : (
...
//         {/* Warnings & proctor info */}
//         <div className="mt-4 p-3 rounded-lg bg-gray-50 border border-gray-100">
//           <p className="text-sm font-medium text-gray-700">Warnings: <span className="font-bold">{warnings}</span> / {MAX_WARNINGS}</p>
//           {!faceDetected && <p className="text-xs text-red-600 mt-1">⚠ Face not detected — warning added</p>}
//           {multipleFaces && <p className="text-xs text-red-600 mt-1">⚠ Multiple faces detected — warning added</p>}
//         </div>
//       </div>
...
//         muted
//         playsInline
//         className="fixed bottom-4 right-4 w-40 h-32 rounded-xl shadow-lg border-2 border-indigo-600 z-50"
//         style={{ objectFit: 'cover' }}
//       />

//       {/* Face warning overlay */}
//       {!faceDetected && !terminated && (
//         <div className="fixed bottom-44 right-4 bg-red-600 text-white p-3 rounded-xl shadow-lg z-50">
//           ⚠ Face Not Detected — Warning Added
//         </div>
//       )}

//       {multipleFaces && !terminated && (
//         <div className="fixed bottom-64 right-4 bg-amber-500 text-white p-3 rounded-xl shadow-lg z-50">
//           ⚠ Multiple faces detected — Warning Added
//         </div>
//       )}
...
//           <div className="bg-white p-8 rounded-xl max-w-lg mx-4 text-center">
//             <h2 className="text-xl font-bold mb-4">Test Terminated</h2>
//             <p className="mb-6">Your test has been terminated due to multiple warnings for suspicious activity.</p>
//             <button className="px-6 py-2 bg-indigo-600 text-white rounded-lg" onClick={() => window.location.reload()}>Reload</button>
//           </div>
//         </div>
//       )}
//     </div>
...
// /* -------------------------
//    ImprovementHighlights, ReportScreen, Main AptitudeTest
...
//       case 'idle':
//       default:
//         return <div className="space-y-8"><PreviousResultsPanel results={previousResults} onViewDetails={handleViewHistory} /><div className="bg-white p-8 rounded-xl shadow-lg text-center w-full md:max-w-8xl mx-auto transition-all duration-300 hover:shadow-xl"><div className="w-16 h-16 bg-indigo-100 rounded-full flex items-center justify-center mx-auto mb-4"><svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-indigo-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" /></svg></div><h2 className="text-2xl font-bold text-gray-900 mb-4">{previousResults && previousResults.length > 0 ? 'Take Another Test' : 'Aptitude Assessment'}</h2><p className="text-gray-600 mb-6">{previousResults && previousResults.length > 0 ? 'Ready to improve your score? Take a new test to track your progress and identify areas of growth.' : 'This test will evaluate your skills across several domains to provide insights into your strengths and areas for improvement.'}</p><button onClick={handleStartTest} className=" w-full px-8 py-4 bg-gradient-to-r from-indigo-200 via-indigo-300 to-purple-200 text-purple-900 font-medium rounded-xl shadow-md transition-colors duration-300 ease-in-out transform-gpu hover:-translate-y-1 hover:shadow-lg hover:from-purple-600 hover:to-purple-700 hover:text-white focus:outline-none focus:ring-4 focus:ring-purple-200 active:scale-95">{previousResults && previousResults.length > 0 ? 'Start New Test' : 'Start Test'}</button></div></div>;
//     }
//   };
...
/* -------------------------
   QuestionScreen without camera/proctoring
   (keeps tab-switch warnings & copy/paste blocking)
   ------------------------- */
...
const QuestionScreen = ({ test, onSubmit, startTime }) => {
  ...
  // Proctoring-related state (only warnings from tab-switch/visibility)
  const [warnings, setWarnings] = useState(0);
  const [terminated, setTerminated] = useState(false);
  ...
  // Terminate on too many warnings (based on tab-switch/visibility)
  useEffect(() => {
    if (warnings >= MAX_WARNINGS) {
      setTerminated(true);
      setTimeout(() => {
  ...
  return (
    <div className="flex flex-col md:flex-row gap-6 w-full">
      ...
      <div className="md:w-[30%] bg-white p-6 rounded-xl shadow-lg h-fit sticky top-4">
        ...
        <div className="mt-4 p-3 rounded-lg bg-gray-50 border border-gray-100">
          <p className="text-sm font-medium text-gray-700">Warnings: <span className="font-bold">{warnings}</span> / {MAX_WARNINGS}</p>
          {/* Only tab-switch/visibility warnings are tracked now */}
          <p className="text-xs text-gray-500 mt-1">Switching tabs or minimizing the window will add a warning.</p>
        </div>
      </div>
...
export default function AptitudeTest() {
  ...
  const handleSubmitTest = async (answers, timeSpent) => {
    ...
    try {
      const response = await axios.post(`/api/aptitude/submit/${testData._1d || testData._id}`, {
        studentId,
        answers,
        timeSpent,
        startTime
