...
const navLinks = [
  { name: 'DASHBOARD', path: '/dashboard' },
  { name: 'RESUME ANALYSIS', path: '/resume-analysis' },
  { name: 'CREATE RESUME', path: '/create-resume' },   // NEW
  { name: 'APPTITUDE CALCULATOR', path: '/aptitude-calculator' },
  { name: 'EXPLORE', path: '/explore' }
];
...
const Header = () => {
  ...
  return (
    <NavContainer>
      ...
      {isAuthenticated && (
        <Hamburger onClick={() => setIsMenuOpen((s) => !s)} aria-label="menu-toggle">
          <FontAwesomeIcon icon={isMenuOpen ? faTimes : faBars} />
        </Hamburger>
      )}
        ...
        <UserProfileContainer ref={dropdownRef}>
          ...
          >
            {user?.avatarUrl ? (
              ...
              <UserInitials>{getInitials(user?.name || 'U')}</UserInitials>
            )}
          </UserProfileButton>

          <DropdownMenu $isOpen={isDropdownOpen}>
            <StyledNavLink to="/profile" onClick={() => setIsDropdownOpen(false)}>
              <DropdownItem>
                <FontAwesomeIcon icon={faUser} />
                Profile
              </DropdownItem>
...
const UserInitials = styled.div`
  ...
  height: 100%;
  border-radius: 50%;
  background: linear-gradient(135deg, ${COLORS.lavender}, ${COLORS.lightAccent});
  display: flex;
  justify-content: center;
  align-items: center;
  font-weight: 700;
